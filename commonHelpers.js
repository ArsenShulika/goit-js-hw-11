...
</li>`).join("");g.innerHTML=r,f.refresh()}const f=new m(".gallery a",{captionsData:"alt",captionDelay:250});let n;const d=document.querySelector(".loader"),h=document.querySelector(".form");document.querySelector(".gallery");h.addEventListener("submit",a=>{if(a.preventDefault(),n=a.target.elements.query.value.trim(),!n){c.error({message:"Sorry, please write correct query!",messageColor:"white",position:"topRight"});return}y(),u(n).then(r=>{if(r.hits.length===0){l(),c.error({title:"Error",position:"topRight",message:"Sorry, there are no images matching your search query. Please, try again!"});return}else l(),p(r.hits)}).catch(r=>{l()})});function y(){d.classList.remove("hidden")}function l(){d.classList.add("hidden")}
